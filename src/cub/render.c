...
/*   Updated: 2022/09/21 00:42:24 by ptopping         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int draw_line(t_data *data, int x)
{
	...
	y = 0;
	while (y < data->game->drawStart)
	{
		my_mlx_pixel_put(data->img, x, y, 0x0066CCFF);//земля
		y++;
	}
	while (y >= data->game->drawStart && y < data->game->drawEnd)
	{
		my_mlx_pixel_put(data->img, x, y, 0x00FF9999);//забор
		y++;
	}
	while (y >= data->game->drawEnd && y < HEIGHT)
	{
...
{
	data->game->camera.x = 2 * x / WIDTH - 1; //x-coordinate in camera space
	...
	data->game->deltaDist.x = fabs(1 / data->game->rayDir.x);
	data->game->deltaDist.y = fabs(1 / data->game->rayDir.y);
	sleep(2);
